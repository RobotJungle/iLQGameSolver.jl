...
function solveILQGame(dynamics, costf, x₀, xgoal, u1goal, u2goal, Q1, Q2, Qn1, Qn2, R11, R12, R21, R22, umin, umax, dmax, ρ, dt, H)
    ...
    û₂ = zeros(k_steps, m2) 
    û = cat(û₁, û₂, dims=2)
    P = rand(k_steps, m1 + m2, n)*0.01
    α = rand(k_steps, m1 + m2)*0.01

    # Rollout players
    xₜ, uₜ = Rollout_RK4(dynamics, x₀, x̂, û, umin, umax, H, dt, P, α, 0.0)

    Aₜ = zeros(Float32, (k_steps, n, n))
    B1ₜ = zeros(Float32, (k_steps, n, m1))
    B2ₜ = zeros(Float32, (k_steps, n, m2))

    Q1ₜ = zeros(Float32, (k_steps, n, n))
    Q2ₜ = zeros(Float32, (k_steps, n, n))

    l1ₜ = zeros(Float32, (k_steps, n))
    l2ₜ = zeros(Float32, (k_steps, n))

    ## CHECK THIS: When changing to agents with different input size
    R11ₜ = zeros(Float32, (k_steps, m1, m1))
    R12ₜ = zeros(Float32, (k_steps, m1, m2))
    R22ₜ = zeros(Float32, (k_steps, m2, m2))
    R21ₜ = zeros(Float32, (k_steps, m2, m1))

    r11ₜ = zeros(Float32, (k_steps, m1))
    r12ₜ = zeros(Float32, (k_steps, m1))
    r22ₜ = zeros(Float32, (k_steps, m2))
    r21ₜ = zeros(Float32, (k_steps, m2))
